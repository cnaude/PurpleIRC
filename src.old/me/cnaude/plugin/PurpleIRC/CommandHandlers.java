    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        ...
        if (args.length >= 1) {
            ...
            if (subCmd.equalsIgnoreCase("leave")) {
                if (args.length >= 3) {
                    String bot = args[1];
                    Channel channel = plugin.ircBots.get(bot).bot.getChannel(args[2]);
                    if (plugin.ircBots.containsKey(bot)) {                        
                        if (args.length >= 4) {
                            String reason = "";
                            for (int i = 3; i < args.length; i++) {
                            ...
                            plugin.ircBots.get(bot).bot.partChannel(channel,reason.substring(1));
                        } else {
                            plugin.ircBots.get(bot).bot.partChannel(channel);
                        }
                    } else {
                        sender.sendMessage(invalidBotName.replaceAll("%BOT%", bot));
                    }
                } else {
                    sender.sendMessage(ChatColor.WHITE + "Usage: " + ChatColor.GOLD + "/irc leave [bot] [channel] ([reason])");
                }
                return true;
            }
            if (subCmd.equalsIgnoreCase("join")) {
                if (args.length >= 3) {
                    ...
                    if (plugin.ircBots.containsKey(bot)) {                        
                        if (args.length == 4) {
                            plugin.ircBots.get(bot).bot.joinChannel(channel,args[3]);
                        } else {
                            plugin.ircBots.get(bot).bot.joinChannel(channel);
                        }
                    } else {
                        sender.sendMessage(invalidBotName.replaceAll("%BOT%", bot));
                    }
                } else {
                    sender.sendMessage(ChatColor.WHITE + "Usage: " + ChatColor.GOLD + "/irc join [bot] [channel] ([password])");
                }
                return true;
            }
